...
const ArticleDetails = () => {
  ...
  const handleDelete = async (id) => {
    navigate(`/opportunities/articles/delete/${id}`);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await api.put(`/articles/${funding._id}`, funding);
  ...
  const handleSaveAndAdd = async () => {
    try {
      ...
      navigate("/opportunities/articles/add");
    } catch (error) {
      console.error("Error saving article:", error);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-md rounded-md">
      <h2 className="text-xl font-semibold mb-4">Change Article</h2>
